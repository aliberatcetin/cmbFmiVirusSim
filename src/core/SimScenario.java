...
public class SimScenario implements Serializable {
	...
	protected static Random rng = new Random(System.currentTimeMillis() / 1000);
	static Coord cord = new Coord((3000.0),3000.0);
	...
	public String getRouter(int virusProbability){
		//return "EpidemicRouter";
		int prob = rng.nextInt(100);
		if(prob<virusProbability){
			return "EpidemicRouter";
		}else{
			return "PassiveRouter";
		}
	}

	public double getRange(int maskProbability){
		int prob = rng.nextInt(100);
		if(prob<maskProbability){
			return 10;
		}else{
			return 15;
		}
	...
	private List<NetworkInterface> getInterface(List<NetworkInterface> interfaces,List<NetworkInterface> interfaces2){
		int interfaceProb = rng.nextInt(100);
		if (interfaceProb < 80) {
			return interfaces;
		}
	...
	private void addInterfaceToArray(List<NetworkInterface> interfaces, int range, Settings s){
		Settings intSettings = new Settings("firstinterface");
		NetworkInterface iface =
				(NetworkInterface)intSettings.createIntializedObject(
						"interfaces.SimpleBroadcastInterface");
		iface.transmitRange = range;
	...
	protected void createHosts() {
		...
		for (int i=1; i<=nrofGroups; i++) {
			...
			addInterfaceToArray(interfaces,1, s);
			addInterfaceToArray(interfaces2,3,s);
			// setup interfaces
			for (int j=1;j<=0;j++) {
			...
			if (mmProto instanceof ScheduledStudentMovement) {
				...
				for (Student student : scheduler.getStudents()) {
					...
					DTNHostStudent host = new DTNHostStudent(this.messageListeners,
							this.movementListeners, gid, getInterface(interfaces, interfaces2), comBus,
							mmProto,currentVirus<totalVirus ? epidemic : passive , this);
					//host.infected = routerName.equals("EpidemicRouter");
					hosts.add(host);
	...
	public void removeHost(DTNHost host){
		hosts.remove(host);
		this.world.getHosts().remove(host);
		//host.setLocation(cord);
	}
