...
abstract public class NetworkInterface implements ModuleCommunicationListener {
    ...
    public static final String TRANSMIT_GROUP_TYPE = "differentRangeGroup";
    ...
    protected DTNHostStudent studentHost = null;
    ...
    public NetworkInterface(Settings s) {
        ...
        if (prob < 10) {
            this.transmitRange = 0.5;
        } else {
            this.transmitRange = 1;
        }


		/*if(finalCounter<5){
			this.transmitRange = 50;
		}else{
			this.transmitRange = s.getDouble(TRANSMIT_RANGE_S);
		}
		finalCounter++;*/
        //String a = s.getSetting("differentRangeGroup");
    ...
    protected boolean isWithinRange(NetworkInterface anotherInterface) {
        ...
        //this student info
        DTNHostStudent dtnHostStudent = (DTNHostStudent) this.host;
        ScheduledStudentMovement.StudentState dtnHostStudentState = dtnHostStudent.getDTNHostState();

        Student dtnStudentInfo = dtnHostStudent.getStudent();
        DTNHostStudent anotherdtnHostStudent = (DTNHostStudent) anotherInterface.host;
        Student anotherdtnStudentInfo = dtnHostStudent.getStudent();
        ScheduledStudentMovement.StudentState anotherdtnHostStudentDTNHostState = anotherdtnHostStudent.getDTNHostState();

        if(dtnHostStudent.getRouter().getClass().equals(PassiveRouter.class)){
            return false;
        }

        if (anotherdtnHostStudentDTNHostState == ScheduledStudentMovement.StudentState.DONE || anotherdtnHostStudentDTNHostState == ScheduledStudentMovement.StudentState.INITIAL || dtnHostStudentState == ScheduledStudentMovement.StudentState.DONE || dtnHostStudentState == ScheduledStudentMovement.StudentState.INITIAL) {
            return false;
        }

        if(dtnHostStudent.getRouter().getClass()==EpidemicRouter.class && anotherdtnHostStudent.getRouter().getClass()==EpidemicRouter.class){
            return false;
        }
        ...
        if(!dtnHostStudent.getRouter().hasAnyMessage()){
            return false;
        }


        if (this.host.getLocation().distance(
                anotherInterface.getHost().getLocation()) > smallerRange) {
            return false;
        }

        if (dtnHostStudentState != ScheduledStudentMovement.StudentState.CLASS) {
            int virusProb = rng.nextInt(10000);
            return virusProb==1;
        } else{
            if (((DTNHostStudent) this.host).getDummyCoord() == null ||
                    anotherdtnHostStudent.getDummyCoord() == null)
                return false;
            else if (dtnStudentInfo.getUpcomingLecture() == anotherdtnStudentInfo.getUpcomingLecture()) {
                int virusProb = rng.nextInt(100000);

                return virusProb==1 && ((DTNHostStudent) this.host).getDummyCoord().distance(anotherdtnHostStudent.getDummyCoord()) <= smallerRange;
            } else {
                return false;
            }
        }
